...
THRESHOLDS_FILE=".github/config/security-thresholds.json"
if [ ! -f "$THRESHOLDS_FILE" ]; then
    echo "❌ Security thresholds configuration not found: $THRESHOLDS_FILE"
    exit 1
fi
...
# Evaluate ESLint Security Results
echo "📋 Evaluating ESLint Security Results..."
if [ -f "eslint-security-report.json" ]; then
    ESLINT_ISSUES=$(cat eslint-security-report.json | jq '[.[] | select(.messages | length > 0)] | length')
    echo "   ESLint Security Issues: $ESLINT_ISSUES"
    
    if [ "$ESLINT_ISSUES" -gt 0 ]; then
        echo "   ⚠️  ESLint security issues found - review required"
        MODERATE_ISSUES=$((MODERATE_ISSUES + ESLINT_ISSUES))
    else
        echo "   ✅ No ESLint security issues found"
    fi
else
    echo "   ❌ ESLint security report not found"
    SECURITY_GATE_PASSED=false
fi

# Evaluate Semgrep Results
echo "📋 Evaluating Semgrep Results..."
if [ -f "semgrep-report.json" ]; then
    SEMGREP_CRITICAL=$(cat semgrep-report.json | jq '[.results[] | select(.extra.severity == "ERROR")] | length')
    SEMGREP_WARNING=$(cat semgrep-report.json | jq '[.results[] | select(.extra.severity == "WARNING")] | length')
    
    echo "   Semgrep Critical Issues: $SEMGREP_CRITICAL"
    echo "   Semgrep Warning Issues: $SEMGREP_WARNING"
...
# Evaluate npm audit Results
echo "📋 Evaluating npm audit Results..."
if [ -f "npm-audit-report.json" ]; then
    NPM_CRITICAL=$(cat npm-audit-report.json | jq -r '.metadata.vulnerabilities.critical // 0')
    NPM_HIGH=$(cat npm-audit-report.json | jq -r '.metadata.vulnerabilities.high // 0')
    NPM_MODERATE=$(cat npm-audit-report.json | jq -r '.metadata.vulnerabilities.moderate // 0')
    ...
    echo "   npm audit High: $NPM_HIGH"
    echo "   npm audit Moderate: $NPM_MODERATE"
    echo "   npm audit Low: $NPM_LOW"
...
# Evaluate Snyk Results
echo "📋 Evaluating Snyk Results..."
if [ -f "snyk-report.json" ]; then
    SNYK_TOTAL=$(cat snyk-report.json | jq '.vulnerabilities | length')
    echo "   Snyk Total Vulnerabilities: $SNYK_TOTAL"
    
    # Snyk vulnerabilities are typically high severity
    HIGH_ISSUES=$((HIGH_ISSUES + SNYK_TOTAL))
    
    if [ "$SNYK_TOTAL" -gt 0 ]; then
        echo "   ⚠️  Snyk vulnerabilities found - review required"
    else
        echo "   ✅ No Snyk vulnerabilities found"
    fi
else
    echo "   ⚠️  Snyk report not found (may be skipped)"
fi

# Evaluate TruffleHog Results
echo "📋 Evaluating TruffleHog Results..."
if [ -f "trufflehog-report.json" ]; then
    VERIFIED_SECRETS=$(cat trufflehog-report.json | jq -s 'map(select(.Verified == true)) | length')
    TOTAL_FINDINGS=$(cat trufflehog-report.json | jq -s 'length')
    
    echo "   TruffleHog Total Findings: $TOTAL_FINDINGS"
    echo "   TruffleHog Verified Secrets: $VERIFIED_SECRETS"
    
    SECRETS_FOUND=$VERIFIED_SECRETS
    
    if [ "$VERIFIED_SECRETS" -gt 0 ]; then
        echo "   ❌ Verified secrets found - CRITICAL SECURITY ISSUE"
        CRITICAL_ISSUES=$((CRITICAL_ISSUES + VERIFIED_SECRETS))
    else
        echo "   ✅ No verified secrets found"
    fi
else
    echo "   ❌ TruffleHog report not found"
    SECURITY_GATE_PASSED=false
fi

# Check thresholds and determine if deployment should be blocked
echo ""
echo "🎯 Security Gate Threshold Evaluation..."
echo "========================================"
...
echo "Moderate Issues: $MODERATE_ISSUES (threshold: $MODERATE_THRESHOLD)"
echo "Low Issues: $LOW_ISSUES (threshold: $LOW_THRESHOLD)"
...
if [ "$MODERATE_ISSUES" -gt "$MODERATE_THRESHOLD" ]; then
    echo "⚠️  Moderate vulnerability threshold exceeded"
    if [ "$(should_block_deployment "moderate")" = "true" ]; then
        SECURITY_GATE_PASSED=false
    fi
fi
...
if [ "$LOW_ISSUES" -gt "$LOW_THRESHOLD" ]; then
    echo "⚠️  Low vulnerability threshold exceeded"
    if [ "$(should_block_deployment "low")" = "true" ]; then
        SECURITY_GATE_PASSED=false
    fi
fi
...
echo "Security summary report generated: security-summary.json"
...
if [ "$SECURITY_GATE_PASSED" = "true" ]; then
    echo "✅ SECURITY GATE PASSED - Deployment can proceed"
    echo "All security checks passed or are within acceptable thresholds"
    exit 0
else
    echo "❌ SECURITY GATE FAILED - Deployment blocked"
    echo "Critical security issues found that exceed acceptable thresholds"
